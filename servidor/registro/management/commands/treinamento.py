...
class Command(BaseCommand):
    ...
    def treinamento_face(self):
        ...
        for coleta in ColetaFaces.objects.all():
           # image_file = coleta.image.url.replace('/media/roi/', '')
            ...
            if not os.path.exists(image_path) or not os.path.isfile(image_path):
                print(f"Caminho não encontrado ou arquivo inválido: {image_path}")
                erro_count += 1
                continue
        ...
        try:
            ...
            model_filename = os.path.join(settings.MEDIA_ROOT, 'classificadorEigen.yml')
            eigenFace.write(model_filename)


            # Salva o modelo no banco de dados
            with open(model_filename, 'rb') as f:
                treinamento, created = Treinamento.objects.get_or_create(id=1)  # Atualiza o modelo existente
                treinamento.modelo.save('classificadorEigen.yml', File(f))

            self.stdout.write(self.style.SUCCESS("TREINAMENTO EFETUADO"))
        except cv2.error as e:
            print(f"Erro do OpenCV durante o treinamento: {e}")
